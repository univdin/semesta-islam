import { NextResponse } from 'next/server';
import { z } from 'zod';
import { getServerIdentity, unauthorizedIdentity } from '@/lib/auth/session';
import { requirePermission } from '@/lib/auth/authorization';
import { CAPABILITIES } from '@/lib/auth/permissions';
import { reverseTransaction } from '@/lib/economy/service';

export const dynamic = 'force-dynamic';

const ReversalSchema = z.object({
  transactionId: z.string().uuid('Invalid transaction UUID'),
  reason: z.string().min(1, 'Reason is required'),
});

export async function POST(request: Request) {
  try {
    const identity = await getServerIdentity();
    if (!identity) {
      return NextResponse.json(unauthorizedIdentity(), { status: 401 });
    }

    // Founder-only governance mutation (not delegable).
    try {
      await requirePermission({ actor: identity, capability: CAPABILITIES.ECONOMY_REVERSAL });
    } catch {
      return NextResponse.json(
        { success: false, statusCode: 403, message: 'Forbidden: economy.reversal is founder-only.' },
        { status: 403 }
      );
    }

    const body = await request.json();
    const parsed = ReversalSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
        {
          success: false,
          statusCode: 400,
          message: 'Validation failed',
          details: parsed.error.errors.map((err) => ({ field: err.path.join('.'), issue: err.message })),
        },
        { status: 400 }
      );
    }

    const { transactionId, reason } = parsed.data;

    const result = await reverseTransaction({ transactionId, actorUserId: identity.userId, reason });

    return NextResponse.json({
      success: true,
      statusCode: 201,
      message: result.duplicate ? 'Transaction already reversed' : 'Economic transaction reversed',
      data: {
        transactionId,
        reversalTransactionId: result.reversalTransaction?.id ?? null,
        status: result.transaction.status,
        reason,
        duplicate: result.duplicate,
      },
    });
  } catch (error: any) {
    return NextResponse.json(
      { success: false, statusCode: 500, message: error?.message ?? 'Internal server error' },
      { status: 500 }
    );
  }
}
